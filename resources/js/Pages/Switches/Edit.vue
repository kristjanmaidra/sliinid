<template>
 <body class="px-4 py-8 flex flex-col items-center justify-center">
        ...
        <p class=" flex justify-center ml-6 mb-3 text-xl font-bold text-blue-900">{{ switches.name }}</p>
...
export default { 
    ...
    setup(props) {
        const form = useForm(props.switches);
        return {form};
    },
    ...
    methods: {
        submit() {
            this.form.post(route('switches.update', this.switches.id));
        },
    },
};
